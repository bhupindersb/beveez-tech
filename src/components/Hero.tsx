...
export default function Hero({
  ...
  return (
    <section className="relative min-h-[85vh] md:min-h-[720px] h-90vh
      ...
      <div className="relative max-w-[1280px] w-full md:px-6 pt-44">
        ...
        <h1 className="font-heading font-bold text-darkBlue leading-tight
               text-[30px] sm:text-[px] md:text-[64px] lg:text-[72px] relative">

          {title}
            {/* Decorative Image around gradient text */}
            {highlightImage && (
              <Image
                src={urlFor(highlightImage).width(976).url()}
                alt=""
                width={976}
                height={191}
